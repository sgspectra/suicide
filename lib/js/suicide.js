...
function initialize(){
    ...
    var gapiplayers = ['Miles', 'Andrew'];
    if (gapi.hangout.getEnabledParticipants() !== undefined) {
        gapiplayers = gapi.hangout.getEnabledParticipants();
    }

    // Create players
    _.each(gapiplayers, function(participantObject){
...
function flipCard(){
    ...
    while (cardInPlayerHand === false) {
        for (var i = 0; i < state.players.length; i++) {
            state.turn = i;
            var scanHand = [state.players[state.turn].round1, state.players[state.turn].round2, state.players[state.turn].round3, state.players[state.turn].round4];
...
// function to generate object for players, which will be used in the shared state
function createPlayerDataObject(players){
    var playerDataObject = '';
    var playerArray = [];
    _.each(players, function(participantObject, index){
        var playerName = participantObject.person.displayName;
        var playerID = participantObject.id;
        // Create a js object for that player
        var playerJSObject = {name :playerName, id :playerID, round1 :'', round2 :'', round3 :'', round4 :'', given :'', taken :'', net :''};
        playerArray.push(playerJSObject);
    });
...
function genSharedState(){
    var state = {deck :deck, turn :0, round :0, players :createPlayerDataObject(players)};
    return state;
}

// TODO on start game button gen the initial shared state and if leader push to SS
