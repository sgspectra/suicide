...
function isLeaderClient(){
    ...
    var leaderID = state.players[0].id;
    var isLeader = false;
    if (myID === leaderID) {
        isLeader = true;
    }
...
$(document).ready(function(){
    ...
    $('#startGame').on('click', function(){
        ...
        // Have lead client send data to shared state via submitDelta
        if (isLeaderClient() === true){
            gapi.hangout.data.submitDelta({'sState' :state});
            gapi.hangout.data.sendMessage('schange');
        }
        ...
        // If it is not a clients turns disable the action buttons
        if (isTurn() === false){
