...
gapi.hangout.onApiReady.add(function(){
    $(document).ready(function(){
        ...
        gapi.hangout.data.onStateChanged.add(function(event){
            ...
            // Parse hHasCard array
            if (state.round >= 4 && state.hHasCard.length > 0) {
                $('#actionButtons').prop('disabled', true);
                shuffleArray(state.hHasCard);
                var playerTurn = state.hHasCard.pop();
                if (state.players[playerTurn].id === gapi.hangout.getLocalParticipant().id) {
                    var hCardData = getNumberAndSuit(state.h[state.h.length - 1]);
                    if (state.giveTakeCard % 2 === 1) {
                        // give card
                        if (state.giveTakeCard === 9) {
                            // TODO Rewrite Inst
                            if (state.hHasCard.length === 0) {
                                state.giveTakeCard++;
                            }

                            giveDrinksTo(hCardData.number * 2);
                        } else {
                            // TODO Rewrite Inst
                            if (state.hHasCard.length === 0) {
                                state.giveTakeCard++;
